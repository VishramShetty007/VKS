...
const EditWForm = ({
  ...
  const [taskData, settaskData] = useState(iniData);
  const [FormTaskData, setFormTaskData] = useState({});
  const [visibleTask, setVisibleTask] = useState(false);

  useEffect(() => {
    taskData_List.current = iniData;
    settaskData(iniData);
  }, [iniData]);
  ...
  const onFinish = (values) => {
    message.success('Workflow edited Successfully.');
    form.resetFields();
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="col-12 w-100 p-0 d-flex justify-content-between h-20 mb-4 align-items-center">
          <Desc text="Notify" classList="fnt-lato fnt-500 fnt-xs mb-2" />
          <Form.Item
            className="mt-3 ml-4"
            name="userName"
            hasFeedback
            initialValue={headData.notify}
            rules={[
      ...
      <WFTask
        ...
        title={
          FormTaskData && Object.keys(FormTaskData).length
            ? 'Edit Task'
            : 'Add Task'
        }
